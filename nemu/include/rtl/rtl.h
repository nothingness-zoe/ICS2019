...
/* RTL basic instructions */
 ...
 inline void interpret_rtl_li(rtlreg_t* dest, uint32_t imm) {
  *dest = imm;
}
...
#define make_rtl_arith_logic(name) \
   ...
   * but we define them here in the same macro */ \
   inline void concat(rtl_, name ## i) (rtlreg_t* dest, const rtlreg_t* src1, int imm) { \
    rtl_li(&ir, imm); \
...
make_rtl_arith_logic(idiv_r)
 ...
 inline void interpret_rtl_idiv64_q(rtlreg_t* dest,
    const rtlreg_t* src1_hi, const rtlreg_t* src1_lo, const rtlreg_t* src2) {
 ...
 inline void interpret_rtl_setrelop(uint32_t relop, rtlreg_t *dest,
 ...
 inline void interpret_rtl_jr(rtlreg_t *target) {
  cpu.pc = *target;
  decinfo_set_jmp(true);
}

 inline void interpret_rtl_jrelop(uint32_t relop,
...
/* RTL pseudo instructions */
 ...
 inline void rtl_sext(rtlreg_t* dest, const rtlreg_t* src1, int width) {
  ...
  /*switch(width) {
    ...
    // 参考 arith.c 中的处理
    case 2: {
      rtl_shli(&t0, src1, (4 - width) * 8);
      rtl_shri(dest, &t0, (4 - width) * 8);
    }
      break;
    case 1: {
      rtl_shli(&t1, src1, (4 - width) * 8);
      rtl_shri(dest, &t1, (4 - width) * 8);
    }
      break;
    default: assert(0);
  }*/
 ...
 inline void rtl_setrelopi(uint32_t relop, rtlreg_t *dest,
 ...
 inline void rtl_mux(rtlreg_t* dest, const rtlreg_t* cond, const rtlreg_t* src1, const rtlreg_t* src2) {
  // dest <- (cond ? src1 : src2)
  TODO();
}
